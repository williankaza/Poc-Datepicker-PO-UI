...
export class AppComponent implements OnInit {
  ...
  apply(formAirfare: FormGroup) {
    const { accompany, destination, endDate, origin, ticketClass, startDate } =
      formAirfare.value;

    const message = `Would you like to confirm the ticket from ${origin} to ${destination} with departure date at
    ${this.getFormatedDate(startDate)} and return at ${this.getFormatedDate(
      endDate
    )} with ${accompany} companions in
    ${ticketClass} class?`;
